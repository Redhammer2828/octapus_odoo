...
class UploadMemberWizard(models.TransientModel):
    ...
    def action_policy_member_upload_excel(self):
        ...
        for index, row in excel_data.iterrows():
            ...
            # If there are errors in the current row, add them to the main errors list
            if row_errors:
                errors.extend(row_errors)
                continue  # Skip processing this row further due to validation errors
        ...
        # Raise an error if any validation errors were found
        if errors:
            error_message = "\n".join(errors)
