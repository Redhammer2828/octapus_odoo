...
class MemberSearch(models.Model):
    ...
    def search_member(self):
        ...
        search_domain = [('member_type', '=', 'policy')]
        ...
        if criteria_count == 0:
            raise UserError("Please enter at least one search criteria to avoid heavy server load.")

        # Reduce the limit for performance
        limit = 200

        # Only fetch IDs for performance
        partner_ids = partner_obj.search(search_domain, order='create_date desc', limit=limit).ids

        if not partner_ids:
            return {
                'name': 'Create Temporary Member',
                'type': 'ir.actions.act_window',
                'res_model': 'create.temporary.member.wizard',
                'view_mode': 'form',
                'target': 'new',
            }
        ...
        return {
            ...
            'type': 'ir.actions.act_window',
            'domain': [('id', 'in', partner_ids), ('membership_state', '!=', 'cancel')],
            'context': {
    ...
    def search_credit_member(self):
        ...
        # Otherwise, open the tree view with the search results
        return {
